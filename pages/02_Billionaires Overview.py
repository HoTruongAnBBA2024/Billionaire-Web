...
with st.container():
    ...
    with col1:
        ...
        # Tạo 2 nút mũi tên trong cột 1, trên 1 hàng ngang
        spacer1, col1_button, col2_button, spacer2 = st.columns([1.5, 1, 1, 1])  # Tạo 2 cột con trong col1


        with col1_button:
            prev_button = st.button("←", on_click=go_prev, key="prev_button_styled", use_container_width=False)
        with col2_button:
            next_button = st.button("→", on_click=go_next, key="next_button_styled", use_container_width=False)
    ...
    with col2:
        ...
        st.markdown(
            f"""### <a href="{billionaire['wiki']}" target="_blank">{billionaire['name']}</a> (Top {st.session_state.page + 1})""",
            unsafe_allow_html=True
        )
